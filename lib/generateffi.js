...
ffiMapType = require('./ffi-type-map');

var templateDir = path.join(__dirname, '..', 'templates', 'ffi');
...
var FFITmpl = hogan.compile(fs.readFileSync(path.join(templateDir, 'ffi.mustache'), 'utf8'));
...
exports.generate = function (opts) {
  ...
  var defineType = function (type) {
    ...
    if (!wrap.abort && wrap.elements.length > 0) {
      wrap.type = StructTmpl.render(wrap);
      wrap.kind = 'struct';
  ...
  */
  ...
  return {
    unmapped: unmapped,
    serialized: FFITmpl.render(toRender),
  };
};
